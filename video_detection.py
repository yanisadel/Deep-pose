...
def graphique_video_position(video, min_detection_confidence=0.7, display=True):
    ...
    Y_moyenne=[[],[],[],[],[],[],[],[]]
    for frame in generateur_decoupe_video(video):
        if(prediction_image_proba(knn_position_dlib,frame,'position', min_detection_confidence)!=None):
            c+=1
            X.append(c)
            prediction,probas=prediction_image_proba(knn_position_dlib,frame,'position', min_detection_confidence)
            for j in range(5):
                Y_temp[j][i]=probas[j]
                Y_moyenne[j].append(sum(Y_temp[j])/10)
            if i==9:
